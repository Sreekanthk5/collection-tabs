document.addEventListener("DOMContentLoaded", function() {
  fetch('https://cdn.shopify.com/s/files/1/0564/3685/0790/files/multiProduct.json') // Replace 'your-api-endpoint-url-here' with your actual API URL
    .then(response => {
      if (!response.ok) {
  ...
  products.forEach(product => {
    ...
    productDiv.innerHTML = `
      ...
      ${
        product.second_image !== 'empty'
          ? `<img src="${product.second_image}" alt="${product.title}" class="product-image" />`
          : ''
      }
      <h3><span class="product_title_name"> ${product.title} </span> . <span class="product_vender">${product.vendor}</span> </h3>
